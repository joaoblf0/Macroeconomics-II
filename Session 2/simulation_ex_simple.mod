...
estimation(
    ...
    first_obs = 1,
   	...
   	presample = 4, % These presample observations do not enter the likelihood, but are used as a training sample

	lik_init = 2, % Type of initialization of Kalman filter
	prefilter = 0, % No prefiltering

	mh_replic = 35000, % Number of iterations for each chain

	mh_nblocks = 3, % Number of parallel chains 

	mh_jscale = 0.350, % Scale parameter of the jumping distribution covariance matrix. Increase if the acceptance ratio is too high.

	mh_drop = 0.5, % The fraction of initially generated parameter vectors to be dropped as a burn-in before using posterior simulations
