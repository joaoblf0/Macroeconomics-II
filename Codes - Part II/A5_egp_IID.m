...
while iter <= max_iter && cdiff>tol_iter
    ...
    for iy = 1:ny
        ...
        % assets today implied by the guess applied tomorrow
        % agrid are the assets tomorrow

        % loop over current period ssets
        for ia  = 1:na 
            if agrid(ia)<ass1(1,iy) %borrowing constraint binds
                % Set asssets tomorrow a_{t+1} = borrowing constraint, 
                % Euler at equality implies a c_t today which require 
                % at least assets ass1(1,iy)
                % Any asset level below ass1(1,iy) must imply a binding
                % constraint (exploiting monotonicity of a_{t+1}(a_t) ).
                sav(ia,iy) = borrow_lim;
                
            else %borrowing constraint does not bind;
                sav(ia,iy) = lininterp1(ass1(:,iy),agrid,agrid(ia));
                % inverting the obtained rule
            end                
        end
...
if ComputeMPC ==1
    ...
    legend('Lowest income state: amount 1','Lowest income state: amount 2',...
        'Highest income state: amount 1','Lowest income state: amount 2',...
    ...
    % save
    print -depsc2 'f_egp_IID_MPCs.eps'  
